...
export async function createProperty(req, res) {
    try {
        ...
        if (!req.body.images || req.body.images.length === 0) {
            ...
            // Asynchronous Portal API calls
            if (newProperty.argenprop) {
                uploadToArgenprop(newProperty).catch(console.error);
            }
            if (newProperty.gvamx) {
                uploadToGvamax(newProperty).catch(console.error);
            }
...
export async function deleteProperty(req, res) {
    try {
        ...
        // Asynchronous Portal API calls for deletion
        if (property.argenprop) {
...
export async function getPublicProperty(req, res) {
    try {
        ...
        res.status(500).json({ message: "Error getting public property" });
    }
}
